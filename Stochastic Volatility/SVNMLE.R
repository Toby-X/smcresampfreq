...
var_smc_m <- function(n,t,alpha,beta,sigma,y){
  ...
  cri[1] = sum(w*seq_dens(cbind(X[,1],x.tmp),alpha,beta,sigma,1,y)*(W/mean(W)*W.tmp/mean(W.tmp)-W.tmp/mean(W.tmp)))
  if (cri[1] > 0){
    idx = sample(1:n,n,prob = w, replace = T)
    X = X[idx,]
    W.u = rep(1/n,n)
  ...
  for (i in 2:(t-1)) {
    ...
    cri[i] = sum(w*seq_dens(cbind(X[,1:i],x.tmp),alpha,beta,sigma,i,y)*(W/mean(W)*W.tmp/mean(W.tmp)-W.tmp/mean(W.tmp)))
    if (cri[i] > 0){
      idx = sample(1:n,n,prob = w, replace = T)
      X = X[idx,]
      W.u = rep(1/n,n)
...
var_smc_s <- function(n,t,alpha,beta,sigma,y){
  ...
  # Resampling
  x.tmp = alpha*X[,1]
  W.tmp = dnorm(y[2],0,beta*exp(alpha*X[,1]/2))
  cri[1] = sum(w*seq_dens(cbind(X[,1],x.tmp),alpha,beta,sigma,1,y)*(W/mean(W)*W.tmp/mean(W.tmp)-W.tmp/mean(W.tmp)))
  if (cri[1] > 0){
    idx = systematic(w)
    X = X[idx,]
...
doit <- function(l){
  ...
  for (i in 1:m) {
    ...
    mse.m = mse.m + (ess.m$estimate-x)^2
    mse.r = mse.r + (ess.r$estimate-x)^2
    mse.s = mse.s + (ess.s$estimate-x)^2
    mse.new.m = mse.new.m + (new.m$estimate-x)^2
    mse.new.r = mse.new.r + (new.r$estimate-x)^2
...
save.image("/public1/home/scf0347/ResampFreq/SV/SVNMLE.RData")
