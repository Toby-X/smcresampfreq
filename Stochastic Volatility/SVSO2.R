...
doit <- function(l){
  ...
  for (l in 1:m) {
    for (j in 1:length(threshold)){
      ...
      w = W/sum(W)
      x.estimate.ss[l,1,j] = sum(w*X[,1]^2)
      ...
      for (i in 2:t) {
        ...
        w = W/sum(W)
        x.estimate.ss[l,i,j] = sum(w*X[,i]^2)
        if (1/sum(w^2)<threshold[j]*n){
          idx = systematic(w)
          X = X[idx,]
          W = rep(1/n,n)
  ...
  for (k in 1:length(threshold)){
    for (i in 1:m)
    {
      mse.ss[k,] = mse.ss[k,]+(x^2-x.estimate.ss[i,,k])^2
    }
  }
  mse.ss = mse.ss/m
...
save.image("/public1/home/scf0347/ResampFreq/SV/SVSO2.RData")