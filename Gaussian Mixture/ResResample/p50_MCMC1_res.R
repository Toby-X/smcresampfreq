...
doit <- function(l){
  ...
  mu7.estimate = rep(0,length(threshold))
  ...
  for (j in 1:length(threshold)) {
    ...
    mu7.estimate[j] = sum(weight*mu7)
    mu10.estimate[j] = sum(weight*mu10)
...
# # tmse = rep(0,m)
# # for (i in 1:m) {
# #   tmse[i] = res[i,]$mse.mu[6]
# # }
# # which.max(tmse)#56
# mse.mu = mse.mu-res[56,]$mse.mu
# mse.lambda = mse.lambda - res[56,]$mse.lambda
# mse.omega = mse.omega - res[56,]$mse.omega
# mse.mu = mse.mu/(m-1)
...
save.image("/public1/home/scf0347/ResampFreq/GaussianMixture/ResResample/p50MCMC1res.RData")
