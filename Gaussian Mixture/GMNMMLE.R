...
newm <- function(){
  ...
  cri[1] = sum(w_n*seq_dens(mu,omega,lambda,1)*(w_u/mean(w_u)*w_u.tmp/mean(w_u.tmp)-w_u.tmp/mean(w_u.tmp)-1))
  ...
  for (i in 2:(p-1)){
    ...
    cri[i-1] = sum(w_n*seq_dens(mu,omega,lambda,1)*(w_u.tmp1/mean(w_u.tmp1)*w_u.tmp/mean(w_u.tmp)-w_u.tmp1/mean(w_u.tmp1)-1))
...
save.image("/public1/home/scf0347/ResampFreq/GaussianMixture/p50MCMC1mulMLE1_0.6.RData")