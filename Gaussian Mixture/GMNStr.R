...
## 这里lognormal multiplicative现在是推导出来的结果
...
newm <- function(){
  ...
  cri[1] = sum(w_n*w_u.tmp*(w_u/mean(w_u)*w_u.tmp/mean(w_u.tmp)-w_u.tmp/mean(w_u.tmp)))
  ...
  for (i in 2:(p-1)){
    ...
    cri[i-1] = sum(w_n*w_u.tmp1*(w_u.tmp1/mean(w_u.tmp1)*w_u.tmp/mean(w_u.tmp)-w_u.tmp1/mean(w_u.tmp1)))
...
save.image("/public1/home/scf0347/ResampFreq/GaussianMixture/p50MCMC1Str.RData")