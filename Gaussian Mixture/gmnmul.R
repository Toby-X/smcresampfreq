...
newm <- function(){
  ...
  cri[1] = sum(w_n*w_u.tmp*(w_u/mean(w_u)*w_u.tmp/mean(w_u.tmp)-w_u.tmp/mean(w_u.tmp)-1))
  ...
  for (i in 2:(p-1)){
    ...
    cri[i-1] = sum(w_n*w_u.tmp1*(w_u.tmp1/mean(w_u.tmp1)*w_u.tmp/mean(w_u.tmp)-w_u.tmp1/mean(w_u.tmp1)-1))
...
# load("p50MCMC1mulMLE1_0.6.RData")
# head(ess)
# head(new)
# 
...
# }
# 
# plot(1:50,colMeans(ess.resamp),"l",ylim=c(0,1))
# par(new = T)
# plot(1:50,colMeans(new.resamp),"l",col="red",ylim = c(0,1))
# 
...
# mean(mse.omega.n)
# 
# sum(sort(mse.lambda.n)<sort(mse.lambda))
# sum(sort(mse.mu.n)<sort(mse.mu))
# sum(sort(mse.omega.n)<sort(mse.omega))
# 
# boxplot(mse.lambda.n)
# boxplot(mse.lambda)

save.image("/public1/home/scf0347/ResampFreq/GaussianMixture/p50MCMC1mul1_0.6.RData")