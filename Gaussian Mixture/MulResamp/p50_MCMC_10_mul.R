...
doit <- function(l){
  ...
  for (j in 1:length(threshold)) {
    ...
    mu7 = c(mu[!idx2,(p-1)*mc+1,2],mu[!idx1,(p-1)*mc+1,1])
    
    omega10 = c(omega[idx1,(p-1)*mc+1,1],omega[idx2,(p-1)*mc+1,2])
    omega7 = c(omega[!idx2,(p-1)*mc+1,2],omega[!idx1,(p-1)*mc+1,1])
    
    lambda10 = c(lambda[idx1,(p-1)*mc+1,1],lambda[idx2,(p-1)*mc+1,2])
    lambda7 = c(lambda[!idx2,(p-1)*mc+1,2],lambda[!idx1,(p-1)*mc+1,1])
