...
doit <- function(l){
  ...
  for (j in 1:length(threshold)) {
    ...
    mu7 = c(mu[!idx2,50,2,j],mu[!idx1,50,1,j])
    
    omega10 = c(omega[idx1,50,1,j],omega[idx2,50,2,j])
    omega7 = c(omega[!idx2,50,2,j],omega[!idx1,50,1,j])
    
    lambda10 = c(lambda[idx1,50,1,j],lambda[idx2,50,2,j])
    lambda7 = c(lambda[!idx2,50,2,j],lambda[!idx1,50,1,j])
...
# mse.mu = mse.mu-res[86,]$mse.mu
# mse.omega = mse.omega-res[86,]$mse.omega
# mse.lambda = mse.lambda-res[86,]$mse.lambda
mse.mu = mse.mu/(m-1)
mse.omega = mse.omega/(m-1)
mse.lambda = mse.lambda/(m-1)
plot(mse.mu)
plot(mse.lambda)
plot(mse.omega)
...
save.image("/public1/home/scf0347/ResampFreq/GaussianMixture/MultiResample/p50MCMC1mul.RData")